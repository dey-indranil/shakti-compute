...
int main() {
  ...
  if (info_result != SHAKTI_SUCCESS) {
    std::cerr << "FAIL: HIP backend should be known\n";
    return 1;
  }

  bool ok = true;
  ok = ok && expect(info.status_message != nullptr,
                    "HIP backend should provide a status message");

  if (!info.available) {
    ok = ok && expect(info.supports_memory == 0,
                      "unavailable HIP backend should not report memory support");
    ok = ok && expect(shaktiIsBackendAvailable("hip") == 0,
                      "unavailable HIP backend should report unavailable");
